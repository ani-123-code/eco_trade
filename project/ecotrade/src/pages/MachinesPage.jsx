...
import { Package, Search, AlertCircle, FileText } from 'lucide-react';
  ...
  const fetchMachines = async () => {
    ...
    try {
      const params = { 
        ...
        limit: 12,
        category: 'machines',
        listingType: 'rfq' // Machines are always RFQ
      };
      if (searchTerm) params.search = searchTerm;
      
      const result = await materialAPI.getAll(params);
      if (result.success) {
        // Filter to only show machines
        const filtered = result.data.filter(m => m.category === 'machines');
        setMachines(filtered);
        setPagination(result.pagination);
      }
    } catch (error) {
      console.error('Failed to fetch machines:', error);
      showError('Failed to load machines. Please try again.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <section className="bg-gradient-to-r from-blue-600 to-blue-700 text-white py-12">
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto text-center">
            <h1 className="text-4xl md:text-5xl font-bold mb-4">Machines Marketplace</h1>
            <p className="text-xl text-gray-200">
              Browse available machines and request quotes from sellers
            </p>
          </div>
        </div>
      </section>
          ...
          <form onSubmit={handleSearch} className="flex gap-4">
            <div className="flex-1">
              <div className="relative">
                ...
                <Input
                  type="text"
                  placeholder="Search machines by name, description..."
                  value={searchTerm}
          ...
          <div className="bg-white rounded-lg shadow-sm p-12 text-center">
            ...
            <p className="text-gray-500 mb-6">
              {searchTerm 
                ...
                : 'There are currently no machines available. Check back later!'}
            </p>
            {searchTerm && (
              <Button
                variant="outline"
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
              {machines.map((machine) => {
                const imageUrl = machine.images && machine.images.length > 0 
                  ? machine.images[0] 
                    ...
                    <div className="p-5">
                      ...
                      <div className="space-y-2 mb-4">
                        <div className="flex items-center text-sm text-gray-600">
                          ...
                          <span>{machine.quantity || 0} {machine.unit || 'unit'}</span>
                        </div>
                        {machine.location && (
                          <div className="flex items-center text-sm text-gray-600">
